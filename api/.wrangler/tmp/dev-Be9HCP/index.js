...
// .wrangler/tmp/bundle-hIa2vL/checked-fetch.js
...
// .wrangler/tmp/bundle-hIa2vL/strip-cf-connecting-ip-header.js
...
async function createToken(user, secret) {
  ...
  const token = await new SignJWT({ sub: user.id, email: user.email }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("7d").sign(secretKey);
  return token;
}
...
scans.post("/", async (c) => {
  const userId = c.get("userId");
  const { domain, sources = "all" } = await c.req.json();
  if (!domain) {
...
// .wrangler/tmp/bundle-hIa2vL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hIa2vL/middleware-loader.entry.ts
